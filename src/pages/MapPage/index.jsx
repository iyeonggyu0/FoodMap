...
const addressToCoords = (address) => {
  return new Promise((resolve, reject) => {
    if (!window.kakao || !window.kakao.maps || !window.kakao.maps.services)
      return reject("Kakao map not loaded");
...
const MapPage = () => {
  ...
  const onDeleteLike = useCallback(
    (ftId) => {
      ...
      axios
        .delete(`${import.meta.env.VITE_API_URL}/map/ft/like?truckId=${ftId}`, {
    ...
    (ftId, day) => {
      ...
      axios
        .post(
          ...
          }/map/ft/sms?truckId=${ftId}&day=${day}`,
          null,
          { withCredentials: true }
        )
        .then(() => {
          ...
          setDetails((prev) => ({
            ...prev,
            schedule: prev.schedule.map((sch) =>
              sch.day === day ? { ...sch, sms: true } : sch
            ),
          }));
        })
        .catch((err) => {
  ...
  const onAddLike = useCallback(
    (ftId) => {
      ...
      axios
        .post(
          `${import.meta.env.VITE_API_URL}/map/ft/like?truckId=${ftId}`,
          null,
          { withCredentials: true }
        )
        .then(() => {
  ...
  const onChangeFilterFun = useCallback(() => {
    ...
    axios
      .get(`${import.meta.env.VITE_API_URL}/map/ft?${params.toString()}`, {
        withCredentials: true,
      })
      .then((res) => {
        if (res.data) {
  ...
  const onChangeFtData = useCallback((data) => {
    ...
    resultArr.forEach((item, idx) => {
      ...
      const todaySchedule = item.schedule.find(
        (sch) =>
          sch.day === dayMap[today] &&
          sch.holiday &&
          sch.mapAddress &&
          sch.mapAddress.trim() !== ""
      );
      ...
      {isMedia.isPc && (
        <div
          ...
          style={{
            position: "absolute",
            right: "0px",
            width: "calc(100vw - min(26vw, 460px))",
            height: "100vh",
          }}
        ></div>
      )}
